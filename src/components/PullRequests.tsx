...
const PullRequests = () => {
  ...
  const getStateIcon = (state: string, merged_at: string | null) => {
    if (state === "open") {
  ...
  if (loading) {
    return (
      <div
        id="pull-requests"
        className="flex flex-col items-center w-full justify-between text-left gap-6 px-4 sm:px-6 lg:px-10"
      >
    ...
    >
      ...
      <div className="flex flex-wrap items-start w-full gap-2 sm:gap-4 lg:gap-6">
        <button
          ...
          className={`px-3 py-2 text-xs sm:text-sm border border-primary w-fit hover:bg-secondary transition-colors ${
            filter === "all" ? "bg-secondary text-primary" : "text-primary"
          }`}
        >
          All (
          {
            pullRequests.filter(
              (pr) =>
                ...
                !blacklistPRs.some(
                  (item) =>
                    item.repo === pr.repository.full_name &&
        ...
        <button
          ...
          className={`px-3 py-2 text-xs sm:text-sm border border-primary w-fit hover:bg-secondary transition-colors ${
            filter === "open" ? "bg-secondary text-primary" : "text-primary"
          }`}
        >
          Open (
          {
        ...
        <button
          ...
          className={`px-3 py-2 text-xs sm:text-sm border border-primary w-fit hover:bg-secondary transition-colors ${
            filter === "closed" ? "bg-secondary text-primary" : "text-primary"
          }`}
        >
          Closed (
          {
